...
class HttpClientHw final : public HttpClient <HttpClientHw>
{
    ...
    public:
        ...
        int  Open             (uint16_t v_len)                                   { return esp_http_client_open               (client, v_len);                    }
        int  FetchHeaders     (void)                                             { return esp_http_client_fetch_headers      (client);                           }
        int  Perform          (void)                                             { return esp_http_client_perform            (client);                           }
        int  Read             (char * v_payload, uint16_t v_len)                 { return esp_http_client_read               (client, (char *)v_payload, v_len); }
        void SetPostField     (const char * v_body, int v_len)                   { esp_http_client_set_post_field            (client, v_body, v_len);            }

    protected:
        void setMethod        (EMethod v_method)                                 { esp_http_client_set_method                (client, static_cast <esp_http_client_method_t> (v_method)); }
