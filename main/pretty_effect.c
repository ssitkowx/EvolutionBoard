...
void pretty_effect_calc_lines(uint16_t *dest, int line, int frame, int linect)
{
    if (frame!=prev_frame) {
        ...
        for (int x=0; x<320; x++) xofs[x]=sin(frame*0.15+x*0.06)*1;
        for (int y=0; y<240; y++) yofs[y]=sin(frame*0.1+y*0.05)*1;
        for (int x=0; x<320; x++) xcomp[x]=sin(frame*0.11+x*0.12)*1;
        for (int y=0; y<240; y++) ycomp[y]=sin(frame*0.07+y*0.15)*1;
        prev_frame=frame;
    }
